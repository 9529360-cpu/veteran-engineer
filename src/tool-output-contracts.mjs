...
const TOOL_OUTPUT_CONTRACTS = Object.freeze({
  ...
  machine_inspect: openObject('Read-only Veteran Machine Bridge inspection result.', {
    operation: stringField('Executed machine inspection operation.'),
    contract: stringField('Machine action contract id.', { minLength: null }),
    enabled: booleanField('Whether machine actions are enabled.'),
    device: anyField('Device identity/platform projection.'),
    allowedLocalRoots: stringArray('Workspace roots accessible to machine actions.'),
    allowedExecutables: stringArray('Executable allowlist for process.start.'),
    limits: anyField('Machine action limits.'),
    sessions: anyField('Session-count or managed session projection.'),
    path: stringField('Resolved workspace path.', { minLength: null }),
    entries: anyField('Directory entries for fs.list.'),
    stat: anyField('Filesystem stat projection for fs.stat.'),
    data: stringField('File data for fs.read.', { minLength: null }),
    digest: stringField('SHA-256 digest for fs.digest.', { minLength: null }),
    ...
    session: anyField('Managed process session projection.'),
    events: anyField('Managed process output events.')
  }, ['operation']),
  machine_act: openObject('Veteran Machine Bridge mutation/execution result.', {
    operation: stringField('Executed machine action operation.'),
    path: stringField('Resolved workspace path.', { minLength: null }),
    destination: stringField('Resolved move destination.', { minLength: null }),
    bytes: integerField('Bytes written or returned.', 0, Number.MAX_SAFE_INTEGER),
    replacements: integerField('Exact replacements applied by fs.replace.', 0, 100),
    created: booleanField('Whether the target path was newly created.'),
    beforeSha256: stringField('Pre-mutation SHA-256 when a regular file existed.', { minLength: null }),
    afterSha256: stringField('Post-mutation SHA-256 when the result is a regular file.', { minLength: null }),
    beforeBytes: integerField('Pre-mutation regular-file byte count.', 0, Number.MAX_SAFE_INTEGER),
    afterBytes: integerField('Post-mutation regular-file byte count.', 0, Number.MAX_SAFE_INTEGER),
    outputDigestContract: stringField('Process output digest contract id for full-stream evidence.', { minLength: null }),
    outputComplete: booleanField('Whether full stdout/stderr stream digests are terminal and complete.'),
    stdoutBytes: integerField('Total stdout bytes observed before text retention truncation.', 0, Number.MAX_SAFE_INTEGER),
    stderrBytes: integerField('Total stderr bytes observed before text retention truncation.', 0, Number.MAX_SAFE_INTEGER),
    stdoutSha256: stringField('SHA-256 over the complete raw stdout byte stream.', { minLength: null }),
    stderrSha256: stringField('SHA-256 over the complete raw stderr byte stream.', { minLength: null }),
    outputSha256: stringField('Aggregate SHA-256 identity derived from complete stdout/stderr stream digests.', { minLength: null }),
    receipt: anyField('Evidence-bound machine action receipt with action/device/result identity.'),
    persistent: booleanField('Whether process.start created an interactive session.'),
    pid: integerField('Started process id.', 1, Number.MAX_SAFE_INTEGER),
    exitCode: integerField('Process exit code.', Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER),
    signal: stringField('Process signal.', { minLength: null }),
    timedOut: booleanField('Whether the process hit the configured timeout.'),
    stdout: stringField('Bounded stdout for one-shot process.start.', { minLength: null }),
    stderr: stringField('Bounded stderr for one-shot process.start.', { minLength: null }),
    session: anyField('Managed process session projection.'),
    sessionId: stringField('Managed process session id.', { minLength: null }),
    signalled: booleanField('Whether process.stop signalled a process tree.'),
    acceptedBytes: integerField('Accepted stdin bytes for process.input.', 0, Number.MAX_SAFE_INTEGER)
  }, ['operation']),
