...
export class VeteranInstaller {
  ...
  async #runtimeDoctor() {
    ...
    let legacy = null;
    if (checks.filter((item) => !item.optional).every((item) => item.ok)) {
    ...
    const runtimePrerequisitesOk = checks.filter((item) => !item.optional && !item.name.startsWith('mcp-')).every((item) => item.ok);
