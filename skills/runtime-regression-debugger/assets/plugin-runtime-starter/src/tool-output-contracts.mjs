...
const TOOL_OUTPUT_CONTRACTS = Object.freeze({
  ...
  machine_inspect: openObject('Read-only Veteran Machine Bridge inspection result.', {
    ...
    repository: anyField('Repository identity and dirty-state projection for repo.status/repo.commands/repo.diff.'),
    observedAt: stringField('Observation timestamp for repo.status/repo.commands/repo.diff.', { minLength: null }),
    commandPlan: anyField('Repository-derived machine command discovery and bounded minimal-validation recommendation for repo.commands.'),
  ...
  machine_act: openObject('Veteran Machine Bridge mutation/execution result.', {
    ...
    outputSha256: stringField('SHA-256 over bounded one-shot stdout and stderr.', { minLength: null }),
