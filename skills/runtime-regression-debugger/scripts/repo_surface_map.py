#!/usr/bin/env python3
"""Print a privacy-safe full-stack surface map for a repository.

Usage: repo_surface_map.py <repo-root>
...
DB_MARKERS = {
    'migrations', 'migration', 'prisma', 'drizzle', 'schema', 'seeds', 'seed',
}

API_DIR_MARKERS = {
    'api', 'routes', 'controllers', 'handlers', 'server', 'functions', 'workers',
}

FRONTEND_DIR_MARKERS = {
    'ui', 'web', 'frontend', 'client', 'pages', 'app', 'components', 'renderer',
}
...
JOB_DIR_MARKERS = {
    'jobs', 'queues', 'queue', 'workers', 'workflows', 'cron', 'scheduler', 'tasks',
}
...
PACKAGE_TECH = {
    ...
    'desktop': {
        'electron', 'electron-builder', '@electron-forge/cli',
    },
    'auth': {
        '@auth/core', 'next-auth', 'passport', 'jose', '@clerk/nextjs', '@clerk/backend',
        'better-auth',
    },
    'contracts': {
        'zod', '@trpc/server', 'graphql', '@apollo/server', 'openapi-types',
        'openapi-typescript', 'protobufjs',
    },
    'feature-flags': {
        '@openfeature/server-sdk', '@openfeature/web-sdk', 'unleash-client',
    ...
    'testing': {
        'playwright', '@playwright/test', 'vitest', 'jest', 'cypress',
        '@testing-library/react', '@testing-library/vue',
    },
    'observability': {
        '@opentelemetry/api', '@sentry/node', '@sentry/electron', '@sentry/react',
        'prom-client', 'pino', 'winston',
    },
}
...
def is_ci_file(path: pathlib.Path, rel: pathlib.Path) -> bool:
    ...
    if str(rel).replace('\\', '/').lower() == '.circleci/config.yml':
        return True
    if lower in {'.gitlab-ci.yml', '.gitlab-ci.yaml', 'jenkinsfile', 'azure-pipelines.yml', 'azure-pipelines.yaml', 'bitbucket-pipelines.yml'}:
        return True
    return False
...
def main() -> int:
    ...
    print('root:', root)
    print('privacy: file names and selected manifest metadata only; .env/secret contents are not read')
    ...
    print_list('Manifests', manifests)

    package_files = [root / rel for rel in manifests if pathlib.Path(rel).name == 'package.json']
    print('\n## package.json technology hints')
    if not package_files:
        print('- none detected')
    for package_file in package_files[:20]:
        info = inspect_package_json(package_file)
        rel = package_file.relative_to(root)
        if info is None:
            print(f'- {rel}: unreadable/invalid JSON')
            continue
    ...
    print_list('Infrastructure/deployment config', infra)
    print_list('CI workflows', workflows)

    monorepo = rels_with_name(paths, MONOREPO_NAMES)
    contracts = sorted(
        str(rel) for path, rel in paths
        if path.is_file() and (path.name in CONTRACT_NAMES or path.suffix.lower() == '.proto')
    )
    ownership_security = sorted(
        str(rel) for path, rel in paths
        if path.is_file() and (path.name in OWNERSHIP_SECURITY_NAMES
                               or (len(rel.parts) >= 3 and rel.parts[0] == '.github'
                                   and rel.parts[1] in {'CODEOWNERS', 'ISSUE_TEMPLATE'}))
    )
    decision_docs = sorted(
        str(rel) for path, rel in paths
        if path.is_file() and any(part.lower() in {'adr', 'adrs', 'decisions', 'architecture'} for part in rel.parts[:-1])
    ...
    print_list('Repository/maintainer instructions', repository_instructions[:80])
    print_list('Monorepo/workspace config', monorepo)
    print_list('Workspace/build graph markers', build_graph[:120])
    print_list('API/schema contract candidates', contracts[:80])
    print_list('Code generation/generated-output markers', codegen[:80])
    print_list('Test runner/config markers', test_configs[:80])
    print_list('Release/version automation markers', release_automation[:80])
    print_list('Ownership/security automation', ownership_security[:80])
    print_list('Architecture/decision docs', decision_docs[:80])

    print_list('Top-level frontend/UI dirs', top_dirs(root, FRONTEND_DIR_MARKERS))
    print_list('Top-level API/backend dirs', top_dirs(root, API_DIR_MARKERS))
    print_list('Top-level job/queue dirs', top_dirs(root, JOB_DIR_MARKERS))
    print_list('Top-level database/migration dirs', top_dirs(root, DB_MARKERS))
    ...
    print_list('Migration/schema candidates', migration_candidates[:80])
    ...
    for path, rel in paths:
        ...
        if name in {'main.cjs', 'main.js', 'main.ts', 'preload.cjs', 'preload.js', 'preload.ts'}:
            ...
            runtime_markers.append(str(rel))
    print_list('Runtime entry/config markers', sorted(set(runtime_markers))[:80])

    print('\n## Next inspection')
    print('- Read repository/maintainer instructions and current deployment/release truth.')
    print('- For release tasks, inspect current release/version workflow contents and determine whether merge, tag, bot, or manual dispatch owns publishing before mutating release state.')
    print('- Trace one requested product contract through registration, active callers, authority, data/async effects, projection, and delivery.')
    print('- Distinguish authoritative source from generated output before editing codegen/schema bindings.')
    print('- Inspect workspace/build graphs and affected targets before whole-repository builds in large monorepos.')
    print('- Inspect public schemas, CODEOWNERS/security policy, test config, and ADRs when the change crosses those boundaries.')
    print('- Treat this map as hints only; file presence does not prove a path is active.')
    return 0
