...
def test_studio_composition_contract():
    ...
    portable_manifest = json.loads((REPO_ROOT / "plugin.json").read_text())
    assert portable_manifest["author"] == {"name": "Veteran Engineer Project"}
    assert '"author": portable.get("author")' in exporter
    ...
    assert "self-modifying revision" in distribution_contract
    ...
    if workflow_path.is_file():
        ...
        assert "pull-requests: read" in package_workflow
        assert "/commits/{sha}/pulls" in package_workflow
        assert 'pr.get("merge_commit_sha") == sha' in package_workflow
        assert "TRUST_ROOTS=(" in package_workflow
        assert "workspace_release_gate.py" in package_workflow
        assert "export_plugin_bundle.py" in package_workflow
        assert "trust_root_changed=true" in package_workflow
        assert "trust_root_changed=false" in package_workflow
        assert "validation-only" not in package_workflow  # workflow message uses explicit publication wording instead
        assert "require a later clean Studio version bump before publication" in package_workflow
