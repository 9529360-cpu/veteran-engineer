...
def test_studio_composition_contract():
    ...
    assert "Publish only the `workspace-install` artifact generated by GitHub Actions on `refs/heads/main`" in distribution_contract
    ...
    assert "scripts/workspace_release_gate.py" in distribution_contract
    assert "immutable SHA-256 digest" in distribution_contract
