...
Do not publish a plugin, package, release, deployment, or completion claim from an unpushed/unreconciled machine tree when the repository is the declared source of truth. Bind distributable artifacts to an exact repository revision and verify that revision after the handoff. Once a distinct host write is independently proven, invalidate stale diffs, CI assumptions, package/release metadata, and cached branch state only where their source identity changed. Do not infer a distinct host merely because the authoritative state advanced.
