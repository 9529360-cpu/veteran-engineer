...
- Publish only the `workspace-install` artifact generated by GitHub Actions on `refs/heads/main`; its `releaseProvenance.sourceRevision` must equal the exact merged main commit being released.
