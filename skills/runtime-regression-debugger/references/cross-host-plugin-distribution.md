...
- If publication reports a release conflict, do not retry by merely substituting the newer release ID. Re-read the installed release and candidate intent. If the intended version/provenance/content is already installed, verify it and stop; otherwise rebuild/reconcile from the new authority before another publish attempt.
