...
- Before calling the publication API, re-read the installed plugin version/release and full source inventory, then run `scripts/workspace_release_gate.py --publication` against the exact local ZIP with the intended main revision, installed version, installed inventory, and immutable GitHub artifact SHA-256 digest. Publication mode must reject wrong profiles, stale/incorrect revisions or versions, local MCP/runtime leakage, unsafe/duplicate/case-colliding ZIP paths, symlinks, manifest disagreement, digest mismatch, and any currently installed path omitted by the candidate because overlay updates cannot delete it. Preserve such paths or ship explicit inert tombstones.
- Immediately before publication, re-read the installed plugin version, current release identity, visibility, and provenance. Bind the update to that exact current release identity.
