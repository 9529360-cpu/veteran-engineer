...
## Workspace Studio publication

Treat Workspace plugin publication as an external release mutation with optimistic concurrency, not as a file copy.

- Publish only the `workspace-install` artifact generated by a `push` on `refs/heads/main` for a real Studio version delta whose exact main SHA is provably the `merge_commit_sha` of exactly one merged pull request targeting `main`. Direct pushes may run validation and produce generic profile artifacts but must not mint a publication candidate. The packaging workflow must also withhold that artifact until the trusted exact-head workflow identities for CI, Skill Engineering, and Cross-platform host smoke have all completed successfully; PR, workflow-dispatch, unchanged-Studio, and generic profile artifacts are never publication candidates. A main push that changes the Workspace publication trust root (the trusted validation workflows, package workflow, Workspace release gate, or exporter) is **validation-only** and must not emit a publishable install artifact from the same self-modifying revision. Land trust-root changes first, then use a later clean Studio version bump so the already-installed prior trust root validates the release revision. Its `releaseProvenance.sourceRevision` must equal that exact clean merged main commit.
- Before using a previously generated candidate, refresh the authoritative GitHub `main` HEAD and compare it with the candidate `releaseProvenance.sourceRevision`. The candidate revision must still be in the current main lineage, current-main `plugin.json.version` must equal the candidate Studio version, and the range from candidate revision to current main must contain no packaged-Skill change and no Workspace publication trust-root change. If any such change exists, the candidate is stale even when its version is newer than the installed plugin; generate a new clean release revision. Unrelated runtime/docs-only commits may advance main without invalidating identical Studio bytes.
- Before calling the publication API, page through the installed plugin source inventory **to completion** (follow every `next_offset` until it is `null`) and capture one installed-state snapshot containing the plugin ID, current release ID, installed version, full canonical path list/count, visited page offsets, and `inventory_complete=true`. Run `scripts/workspace_release_gate.py --publication --installed-state <snapshot>` against the exact local ZIP with the intended main revision and immutable GitHub artifact SHA-256 digest. Publication mode must reject incomplete pagination, snapshot/version disagreement, wrong profiles, stale/incorrect revisions or versions, local MCP/runtime leakage, unsafe/duplicate/case-colliding ZIP paths, symlinks, manifest disagreement, digest mismatch, and any currently installed path omitted by the candidate because overlay updates cannot delete it. Preserve such paths or ship explicit inert tombstones.
- Treat the gate result's `expected_release_id` as part of the publication proof. Immediately before publication, re-read installed version/release/visibility/provenance; if the release identity differs from that gated snapshot, discard the gate result and rebuild it from a fresh complete installed-state snapshot. Pass the unchanged gated release ID as the publication API's optimistic-concurrency baseline.
- If publication reports a release conflict, do not retry by merely substituting the newer release ID. Re-read the installed release and candidate intent. A conflict proves only that the release state changed since the bound read; it does not prove a different human, model, session, or host performed the change. First reconcile live tool-return identities still present in the active execution, then revalidate any persisted historical publish receipts and platform-bound automation they may have triggered. If the intended version/provenance/content is already installed, verify it and stop; otherwise rebuild/reconcile from the new authority before another publish attempt.
- After publication, read back the root manifest, distribution provenance, Skill registration metadata, and any changed compatibility/tombstone files before claiming the release is synchronized.
- Workspace updates are overlays. A removed source file remains online unless the platform supports deletion. Neutralize unavoidable historical paths explicitly and keep clean-package/installed inventories reconcilable.

This publication contract is separate from runtime GitHub Releases and from Desktop/Codex runtime versioning.
