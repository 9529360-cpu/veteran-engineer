...
- Before calling the publication API, run `scripts/workspace_release_gate.py` against the exact local ZIP with the intended main revision and Studio version; when the artifact service exposes an immutable SHA-256 digest, pass it too. The gate must reject wrong profiles, stale/incorrect revisions or versions, local MCP/runtime leakage, unsafe/duplicate ZIP paths, symlinks, manifest disagreement, and digest mismatch.
