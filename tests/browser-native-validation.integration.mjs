...
test('Remote Host MCP drives real Chromium UI-to-API validation with verified screenshot evidence', { skip: !browserPath, timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const initialized = await initRemoteHostConfig({ configPath, stateRoot, workspaces: [repo], port: 0 });
    running = await startRemoteHost({ configPath });
    const { Client, StreamableHTTPClientTransport } = await import('@modelcontextprotocol/client');
    client = new Client({ name: 'veteran-remote-browser-proof', version: '1.0.0' });
    await client.connect(new StreamableHTTPClientTransport(new URL(running.endpoint), {
    ...
    const health = await call('runtime_health', {});
    assert.equal(health.surface.id, 'secure-tunnel');
    const project = await call('project_open', { requestId: randomUUID(), repoPath: repo });
    const result = await call('validation_run', { requestId: randomUUID(), projectId: project.id, capability: 'native-browser-fullstack' });
    ...
    const [evidence] = await call('evidence_query', { projectId: project.id, ids: [result.evidenceId] });
